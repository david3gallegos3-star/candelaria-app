// ============================================
// TrazabilidadHeader.js
// Header sticky del módulo de trazabilidad
// ============================================
import React from 'react';

const TABS = [
  { key: 'lotes',    emoji: '🏷️', label: 'Lotes'       },
  { key: 'calidad',  emoji: '✅', label: 'Calidad'     },
  { key: 'arcsa',    emoji: '📋', label: 'ARCSA'       },
  { key: 'rastreo',  emoji: '🔍', label: 'Rastreo'     },
];

export default function TrazabilidadHeader({
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: mobile ? 6 : 8 }}>
          ...
          <div style={{ color: 'white', fontWeight: 'bold', fontSize: mobile ? '14px' : '17px' }}>
            🏷️ Trazabilidad
          </div>
        </div>
      </div>
      <div style={{
        ...
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTabActiva(t.key)} style={{
            flex: 1, padding: mobile ? '8px 4px' : '9px 8px',
            border: 'none', borderRadius: '7px', cursor: 'pointer',
            fontSize: mobile ? '11px' : '12px', fontWeight: 'bold',
            background: tabActiva === t.key ? 'rgba(255,255,255,0.2)' : 'transparent',
