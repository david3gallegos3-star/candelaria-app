...
const animationVariants = {
  ...
  calmInUp: {
    container: {
      hidden:  {},
      visible: (i = 1) => ({ transition: { staggerChildren: 0.01, delayChildren: 0.2 * i } }),
    },
    child: {
  ...
  rollIn: {
    ...
    child: {
      ...
      visible: { opacity: 1, y: '0em', transition: { duration: 0.65, ease: [0.65, 0, 0.75, 1] } },
    },
  },
  whipIn: {
    container: { hidden: {}, visible: {} },
    child: {
      ...
      visible: { opacity: 1, y: '0em', transition: { duration: 0.45, ease: [0.85, 0.1, 0.9, 1.2] } },
    },
  },
};

export function TextAnimate({ text, type = 'whipInUp', style, className, ...props }) {
        ...
        {text.split(' ').map((word, i) => (
          ...
          >
            {word.split('').map((char, j) => (
              <motion.span key={j} variants={child} style={{ display: 'inline-block', marginRight: '-0.01em' }}>
                {char}
              </motion.span>
            ))}
          </motion.span>
