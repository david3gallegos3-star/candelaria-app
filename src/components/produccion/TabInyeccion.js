// ============================================
// TabInyeccion.js — Inyección de salmuera (tab dentro de Producción)
// ============================================
import React, { useState, useEffect, useCallback } from 'react';
import { supabase } from '../../supabase';

export default function TabInyeccion({ currentUser, mobile }) {
  ...
  const kgSalmueraReq   = kgCarneTotal;
  ...
  async function guardarProduccion() {
    ...
    try {
      ...
      for (const ing of ingredientesExp) {
        ...
        if (invR) await supabase.from('inventario_mp').update({ stock_kg: Math.max(0, parseFloat(invR.stock_kg) - ing.kgUsados) }).eq('materia_prima_id', ing.materia_prima_id);
      }
      for (const c of filasCorte) {
        ...
        if (invR) await supabase.from('inventario_mp').update({ stock_kg: Math.max(0, parseFloat(invR.stock_kg) - c.kg_carne_cruda) }).eq('materia_prima_id', c.materia_prima_id);
      }
  ...
  async function revertirLote(lote) {
    ...
    try {
      for (const ing of (lote.produccion_inyeccion_ingredientes || [])) {
        ...
        if (invR) await supabase.from('inventario_mp').update({ stock_kg: nuevoStock }).eq('materia_prima_id', ing.materia_prima_id);
        else await supabase.from('inventario_mp').insert({ materia_prima_id: ing.materia_prima_id, nombre: ing.ingrediente_nombre, stock_kg: parseFloat(ing.kg_usados), stock_minimo_kg: 0 });
      }
    ...
    try {
      for (const f of filasRetazos) {
        ...
        await supabase.from('produccion_inyeccion_cortes').update({
          kg_retazos: f.kg_retazos, precio_venta_retazo_kg: f.precio_venta_retazo_kg,
          ingreso_retazos: ingRetr, kg_carne_limpia: kgLimpia, costo_final_kg: costoFinalKg,
        }).eq('id', f.id);
      }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', background:'white', borderRadius:10, padding:4, marginBottom:14, gap:4, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        {[['nueva','💉 Nueva producción'],['lotes','📋 Lotes']].map(([key, label]) => (
          <button key={key} onClick={() => setSubTab(key)} style={{
            flex:1, padding: mobile ? '8px 4px' : '9px 12px', border:'none', borderRadius:7, cursor:'pointer',
            fontSize: mobile ? '11px' : '13px', fontWeight:'bold',
            background: subTab === key ? '#1a3a5c' : 'transparent',
            color:      subTab === key ? 'white'   : '#666', transition:'all 0.2s'
          }}>{label}</button>
        ))}
      </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ background:'white', borderRadius:12, padding:16, boxShadow:'0 2px 8px rgba(0,0,0,0.07)' }}>
              ...
              {formulasSalmuera.length === 0 ? (
                ...
                <>
                  <select value={formulaSelec?.id || ''} onChange={e => setFormulaSelec(formulasSalmuera.find(x => x.id === e.target.value) || null)}
                    ...
                    {formulasSalmuera.map(f => <option key={f.id} value={f.id}>{f.nombre}</option>)}
                  </select>
                  {formulaSelec && (
            ...
            <div style={{ background:'white', borderRadius:12, padding:16, boxShadow:'0 2px 8px rgba(0,0,0,0.07)' }}>
              ...
              <div style={{ marginTop: filasCort.length > 0 ? 8 : 0 }}>
                <input placeholder={filasCort.length === 0 ? '🔍 Buscar materia prima para agregar...' : '➕ Agregar otro corte — escribe para buscar...'}
              ...
              {filasCort.length === 0 && !buscadorCorte && (
                <div style={{ textAlign:'center', padding:'16px', color:'#aaa', fontSize:12 }}>Escribe arriba para buscar y agregar materias primas a inyectar</div>
              )}
            </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ background:'#1a3a5c', borderRadius:12, padding:16 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {[['Total kg cortes', `${kgCarneTotal.toFixed(3)} kg`, 'white'],['Salmuera a preparar', `${kgSalmueraReq.toFixed(3)} kg`, '#4fc3f7']].map(([l, v, col]) => (
              ...
              {formulaSelec && kgCarneTotal > 0 && (
                <div style={{ marginTop:10, fontSize:11, color:'rgba(255,255,255,0.6)', textAlign:'center' }}>Fórmula escalada × {kgCarneTotal.toFixed(3)} kg</div>
              )}
            </div>

            {formulaSelec && ingredientesExp.length > 0 && (
              <div style={{ background:'white', borderRadius:12, padding:16, boxShadow:'0 2px 8px rgba(0,0,0,0.07)' }}>
                <h4 style={{ margin:'0 0 10px', color:'#1a1a2e', fontSize:14, borderBottom:'2px solid #8e44ad', paddingBottom:6 }}>🧪 Hoja del Formulador</h4>
                <div style={{ fontSize:11, marginBottom:8, padding:'6px 10px', borderRadius:6, background: kgSalmueraReq > 0 ? '#eaf4fb' : '#fff9e6', color: kgSalmueraReq > 0 ? '#1a3a5c' : '#b7770d' }}>
                  {kgSalmueraReq > 0
                ...
                {ingredientesExp.map((ing, i) => (
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'7px 0', borderBottom:'1px solid #f5f5f5', fontSize:12 }}>
                    <div>
                      ...
                      <div style={{ color: ing.stockOk ? '#aaa' : '#e74c3c', fontSize:10 }}>Stock: {ing.stockKg.toFixed(3)} kg {!ing.stockOk && '⚠️ insuf.'}</div>
                    </div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#888', marginTop:4 }}>
                  <span>Costo salmuera/kg</span><span>${costoSalmuera_kg.toFixed(4)}/kg</span>
                </div>
              </div>
            )}
            ...
            {formulaSelec && cortesConCosto.filter(c => parseFloat(c.kg) > 0).length > 0 && (
              <div style={{ background:'white', borderRadius:12, padding:16, boxShadow:'0 2px 8px rgba(0,0,0,0.07)' }}>
                <h4 style={{ margin:'0 0 10px', color:'#1a1a2e', fontSize:14, borderBottom:'2px solid #27ae60', paddingBottom:6 }}>📊 Costo por Corte</h4>
                {cortesConCosto.filter(c => parseFloat(c.kg) > 0).map((c, i) => (
                  <div key={i} style={{ background:'#f8f9fa', borderRadius:8, padding:'10px 12px', marginBottom:8 }}>
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6, fontSize:11 }}>
                      {[['Carne',`$${c.costoCarne.toFixed(3)}`,'#e74c3c'],['Salmuera',`$${c.costoSal.toFixed(3)}`,'#8e44ad'],['Total',`$${c.costoTotal.toFixed(3)}`,'#1a3a5c']].map(([l,v,col]) => (
              ...
              <div key={lote.id} style={{ background:'white', borderRadius:14, marginBottom:16, boxShadow:'0 2px 8px rgba(0,0,0,0.07)', overflow:'hidden' }}>
                <div style={{ background:'#1a3a5c', padding:'10px 16px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div>
                    ...
                    <div style={{ color:'rgba(255,255,255,0.6)', fontSize:11 }}>{parseFloat(lote.kg_carne_total).toFixed(2)} kg · #{lote.id.slice(-6).toUpperCase()}</div>
                  </div>
