...
ALTER TABLE caja_gastos ADD COLUMN IF NOT EXISTS origen_nomina_movimiento_id uuid REFERENCES nomina_movimientos(id);

-- Misma marca para el caso "banco" -- permite borrar el pago vinculado si
-- se borra el anticipo desde Nomina.
ALTER TABLE talonario_pagos_banco ADD COLUMN IF NOT EXISTS origen_nomina_movimiento_id uuid REFERENCES nomina_movimientos(id);
